...
public :: picker_utils, multi_pick
private
...
logical, parameter :: L_WRITE      = .false.
    ...
    contains
        ...
        subroutine multi_pick( fromto_diam, npickers, micname, pcontrast, smpd, boxfile, nptcls, dir_out, mic_stats, ndev)
            real,                       intent(in)    :: fromto_diam(2)
            ...
            real,                       intent(out)   :: mic_stats(params_glob%nmoldiams,4)
            character(len=*), optional, intent(in)    :: dir_out
            integer,                    intent(out)   :: nptcls
            integer,                    intent(in)    :: npickers
            character(len=LONGSTRLEN),  intent(out)   :: boxfile
            type(picker_utils), allocatable  :: pickers(:)
            real                             :: stepsz, moldiam, pick_stats(4)
            integer                          :: ipick
            !real, allocatable                :: pick_stats(:)
            allocate(pickers(npickers))
            moldiam  = fromto_diam(1) ! initialize moldiam
            stepsz = (fromto_diam(2) - fromto_diam(1)) / (npickers - 1)
            do ipick = 1, npickers
                !allocate(pick_stats(4))
                call pickers(ipick)%new(micname, pcontrast, smpd, moldiam)
                call pickers(ipick)%exec_picker(boxname_out=boxfile, nptcls=nptcls, dir_out=dir_out, pick_stats=pick_stats) 
                mic_stats(ipick,:) = pick_stats
                moldiam = moldiam + stepsz ! update moldiam here
                !deallocate(pick_stats)
                pick_stats = 0.
            end do
            deallocate(pickers)
        end subroutine multi_pick
        ...
        subroutine exec_picker( self, boxname_out, nptcls, dir_out, pick_stats )
            class(picker_utils),        intent(inout) :: self
            character(len=LONGSTRLEN),  intent(out)   :: boxname_out
            integer,                    intent(out)   :: nptcls
            character(len=*), optional, intent(in)    :: dir_out
            real, optional            , intent(out)   :: pick_stats(4)
            real    :: shrink2
            integer :: box
            ...
            call self%peak_vs_nonpeak_stats( pick_stats ) 
            ! # ptcls
            nptcls          = self%nboxes2
        ...
        subroutine peak_vs_nonpeak_stats( self, pick_stats )
            class(picker_utils), intent(inout) :: self
            real,                intent(out)   :: pick_stats(4)
            integer, allocatable :: positions_backgr(:,:)
            real,    allocatable :: scores_nonpeak(:)
            type(image) :: boximgs_heap(nthr_glob)
            real        :: scores_peak(self%nboxes2), factor, smd, ksstat, prob
            real        :: scores(self%nrefs), pixrad_shrink2, rpos(2), a_peak, a_nonpeak, s_peak, s_nonpeak, moldiam_print
            ...
            moldiam_print = self%maxdiam / 1.111
            write(logfhandle,'(a,1x,f5.3)') 'SMD           = ', smd
            write(logfhandle,'(a,1x,f5.3)') 'K-S statistic = ', ksstat
            write(logfhandle,'(a,1x,f5.3)') 'P             = ', prob
            write(logfhandle,'(a,1x,f6.2)') 'MOLDIAM = ', moldiam_print
    
            pick_stats = [moldiam_print, smd, ksstat, a_peak]

...
module simple_math
    ...
    contains
        ...
        !> finds local maxima based on xcoords and ycoords input vectors
        ...
        function find_local_maxima(xcoords, ycoords, nentries) result(maxima)
            ...
            ! check for absolute max (both endpoints)
            maxloc_front = maxloc(ycoords)
            maxloc_back = maxloc(ycoords,back=.true.)
            if (.not. any(locmax .eq. maxloc_front(1)) ) then
                nmaxima = nmaxima + 1
                locmax(nmaxima) = maxloc_front(1)
            end if
            if (.not. any(locmax .eq. maxloc_back(1))) then
                nmaxima = nmaxima + 1
                locmax(nmaxima) = maxloc_back(1)
            end if

...
use simple_picker_utils, only: picker_utils, multi_pick
...
#include "simple_local_flags.inc"
    ...
    contains
        ...
        subroutine iterate( self, cline, smpd, moviename_intg, boxfile, nptcls_out, dir_out, mic_stats,fromto_diam, nmoldiams )
            ...
            real,    optional,              intent(out)   :: mic_stats(params_glob%nmoldiams,4) 
            real,    optional,              intent(in)    :: fromto_diam(2)
            integer, optional,              intent(in)    :: nmoldiams  
            ...
            select case(trim(params_glob%picker))
                ...
                case('new')
                    ...
                    if (params_glob%multipick .eq. 'yes') then
                        call multi_pick(fromto_diam, nmoldiams, moviename_intg, params_glob%pcontrast, smpd, boxfile, nptcls_out, dir_out, mic_stats)
                    else
                        if (.not. present(fromto_diam)) then
                            call putils%new(moviename_intg, params_glob%pcontrast, smpd, params_glob%moldiam)
                        else
                            call putils%new(moviename_intg, params_glob%pcontrast, smpd, fromto_diam(1))
                        end if
                        ...
                        call putils%exec_picker(boxfile, nptcls_out, dir_out=dir_out, pick_stats=mic_stats(1,:) )
                        call putils%kill
                    endif
            end select

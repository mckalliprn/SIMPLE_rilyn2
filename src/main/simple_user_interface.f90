...
logical, parameter :: DEBUG = .false.
    ...
    type(simple_program), target :: multipick ! new
    ...
    contains
        ...
        subroutine set_prg_ptr_array
            ...
            call push2prg_ptr_array(multipick) !new
        ...
        ! new
        subroutine new_multipick
            call multipick%new(&
            &'multipick',&                                                                          ! name
            &'particle picking overmultiple molecular diameters',&                                  ! descr_short
            &'is a program to find optimal molecular diameter for automatic particle picking',&     ! descr_long
            &'simple_exec',&                                                                        ! executable
            ...
            ! INPUT PARAMETER SPECIFICATIONS
            ! image input/output
            call multipick%set_input('img_ios', 1, 'pickrefs', 'file', 'Stack of class-averages/reprojections for picking', 'Stack of class-averages/reprojections for picking', 'e.g. pickrefs.mrc', .false., '')
            call multipick%set_gui_params('img_ios', 1, submenu="picking", advanced=.false.)
            ! parameter input/output
            call multipick%set_input('parm_ios', 1, 'dir', 'dir', 'Output directory', 'Output directory', 'e.g. pick/', .false., 'pick')
            call multipick%set_gui_params('parm_ios', 1, submenu="picking")
            call multipick%set_input('parm_ios', 2, pcontrast)
            call multipick%set_gui_params('parm_ios', 2, submenu="picking")
            call multipick%set_input('parm_ios', 3, moldiam)
            call multipick%set_gui_params('parm_ios', 3, submenu="picking")
            call multipick%set_input('parm_ios', 4, picker)
            call multipick%set_gui_params('parm_ios', 4, submenu="picking")
            ! alternative inputs
            ! <empty>
            ! search controls
            call multipick%set_input('srch_ctrls', 1, 'thres', 'num', 'Distance threshold in Angs','Distance filter in Angs{24}', '{24}', .false., 24.)
            call multipick%set_gui_params('srch_ctrls', 1, submenu="picking", advanced=.false.)
            call multipick%set_input('srch_ctrls', 2, 'ndev', 'num', '# of sigmas for outlier detection', '# of standard deviations threshold for outlier detection{2.5}', '{2.5}', .false., 2.5)
            call multipick%set_gui_params('srch_ctrls', 2, submenu="picking", advanced=.false.)
            call multipick%set_input('srch_ctrls', 3, pgrp)
            call multipick%set_gui_params('srch_ctrls', 3, submenu="picking", advanced=.false.)
            multipick%srch_ctrls(3)%required = .false.
            ! filter controls
            call multipick%set_input('filt_ctrls', 1, 'lp', 'num', 'Low-pass limit','Low-pass limit in Angstroms{20}', 'in Angstroms{20}', .false., 20.)
            call multipick%set_gui_params('filt_ctrls', 1, submenu="picking")
            ! mask controls
            ! <empty>
            ! computer controls
            call multipick%set_input('comp_ctrls', 1, nparts)
            call multipick%set_gui_params('comp_ctrls', 1, submenu="compute", advanced=.false.)
            call multipick%set_input('comp_ctrls', 2, nthr)
            call multipick%set_gui_params('comp_ctrls', 2, submenu="compute", advanced=.false.)
        end subroutine new_multipick
        ...
        subroutine new_pick
            ...
            &'simple_exec',&                                                   ! executable
            &1, 4, 3, 3, 1, 0, 2, .true.)                                      ! # entries in each group, requires sp_project
            pick%gui_submenu_list = "picking,compute"
            pick%advanced = .false.
            ! INPUT PARAMETER SPECIFICATIONS
            ! image input/output
            ...
            ! alternative inputs
            !!! NEW, also not sure if best category to put it in
            call pick%set_input('alt_ios', 1, 'multipick', 'character', 'Multiple runs of pick', 'Multiple runs of pick using different molecular diameters', '(yes|no)', .false., 'no')
            call pick%set_gui_params('alt_ios', 1, submenu="picking")
            call pick%set_input('alt_ios', 2, 'nmoldiams', 'num', 'Number of molecular diameters to investigate', 'Number of molecular diameters tested', 'e.g. 5', .false., 5.)
            call pick%set_gui_params('alt_ios', 2, submenu="picking")
            call pick%set_input_1('alt_ios', 3, 'moldiam_up', 'num', 'Upper bound molecular diameter in multipick', 'Upper bound molecular diameter in multipick', 'e.g. 200', .false., 200.)
            call pick%set_gui_params('alt_ios', 3, submenu="picking")
            !!! 
            ! search controls
